<template>
  <div class="q-pa-md">
    ...
    >
      <template v-slot:top>
        <q-btn color="yellow-7" icon="add" :disable="loading" :label="$q.screen.xs || $q.screen.sm ? '' : 'Add Block'" @click="submit('Add')" />
        <q-space />
      ...
      <template v-slot:body="props">
        <q-tr :props="props">
          ...
          <q-td auto-width>
            <q-btn size="sm" color="red" :key="props.row.id" :loading="deleteLoading[`deleteLoading-${props.row.id}`]" round dense icon="delete" @click="deleteBlock(props.row.id)" />
          </q-td>
        </q-tr>
      </template>
...
export default {
  setup () {
    ...
    async function onRequest (props) {
      loading.value = true
      let blocks = await Api.getList('blocks');
    ...
    async function deleteBlock(id){
      ...
      let res = await Api.delete('blocks',{id})
      if (res.data.success){
